// Đào tạo xử lý đổi lịch — port `lichgiangdaotao.html` + `.js`. Tương tự "Khoa xác
// nhận đổi lịch" nhưng:
//  - Có thêm filter Khoa quản lý
//  - List dùng PhamVi_XL, học phần + người gửi cascade qua khoa
//  - Trạng thái duyệt lấy từ master data DUYETDOILICH
//  - Duyệt dùng action Them_TKB_XacNhanDoiLich
...
import {
  ...
  YeuCauDoiLichItem,
  ChiTietDoiLichItem,
} from '../../services/giangVien/lecturerScheduleApprovalService';
...
const LecturerScheduleProcessingScreen = () => {
  const navigation = useNavigation();

  // Filters
  const [hocKyList, setHocKyList] = useState<HocKyItem[]>([]);
  const [khoaQuanLyList, setKhoaQuanLyList] = useState<TrangThaiItem[]>([]);
  ...
  const [khoaQuanLyId, setKhoaQuanLyId] = useState('');
  ...
  // Init: học kỳ + 2 master data DM
  useEffect(() => {
    (async () => {
      try {
        const [hk, ttd, kqxl] = await Promise.all([
          svc.getHocKy(),
          svc.getTrangThaiDuyetMaster(),
          svc.getKetQuaXuLy(),
        ]);
        setHocKyList(hk);
  ...
  // Đổi học kỳ → reload khoa + HP + người gửi
  useEffect(() => {
    if (!hocKyId) return;
    (async () => {
      setLoadingFilters(true);
      try {
        const [khoas, hp, ng] = await Promise.all([
          svc.getKhoaQuanLy(hocKyId),
          svc.getHocPhanXuLy(hocKyId, ''),
          svc.getNguoiGuiXuLy(hocKyId, '', ''),
        ]);
        setKhoaQuanLyList(khoas);
        setHocPhanList(hp);
        setNguoiGuiList(ng);
        setKhoaQuanLyId('');
        setHocPhanId('');
        ...
        console.warn('[Processing] reload by hocKy:', e?.message);
      } finally {
  ...
  // Đổi khoa quản lý → reload HP + người gửi
  useEffect(() => {
    if (!hocKyId) return;
    (async () => {
      setLoadingFilters(true);
      try {
        const [hp, ng] = await Promise.all([
          svc.getHocPhanXuLy(hocKyId, khoaQuanLyId),
          svc.getNguoiGuiXuLy(hocKyId, khoaQuanLyId, ''),
        ]);
        setHocPhanList(hp);
        setNguoiGuiList(ng);
        setHocPhanId('');
        ...
        console.warn('[Processing] reload by khoa:', e?.message);
      } finally {
        setLoadingFilters(false);
      }
    })();
    // Chạy chỉ khi khoaQuanLyId thực sự đổi (skip first time vì đã handle ở hocKy effect)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [khoaQuanLyId]);

  // Đổi HP → reload người gửi
  useEffect(() => {
    if (!hocKyId) return;
    (async () => {
      try {
        const ng = await svc.getNguoiGuiXuLy(hocKyId, khoaQuanLyId, hocPhanId);
        setNguoiGuiList(ng);
        setNguoiGuiId('');
      } catch (e: any) {
        console.warn('[Processing] reload nguoiGui by hocPhan:', e?.message);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hocPhanId]);
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      const res = await svc.getListXuLy({
        hocKyId,
        khoaQuanLyId,
        tuNgay,
        denNgay,
  ...
  const openDuyetSheet = () => {
    ...
    if (ketQuaXuLyList.length === 0) {
      Alert.alert('Lỗi', 'Chưa có danh sách trạng thái xử lý');
      return;
    }
    setDuyetNoiDung('');
  ...
  // Đào tạo dùng dropPhetDuyet_TrangThai2 = danh mục XACNHANDOILICH (=== ketQuaXuLyList)
  const submitDuyet = async (tinhTrangId: string) => {
    ...
    for (const id of ids) {
      try {
        await svc.duyetXuLy({ sanPhamId: id, tinhTrangId, noiDung: duyetNoiDung });
        ok++;
      } catch {
        failed++;
      }
    ...
    Alert.alert(
      'Kết quả',
      `Xử lý thành công: ${ok}/${ids.length}` + (failed ? `\nThất bại: ${failed}` : ''),
  ...
  const selKhoa = khoaQuanLyList.find((it) => it.ID === khoaQuanLyId);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Đào tạo xử lý đổi lịch</Text>
          <Text style={styles.headerSubtitle}>Xử lý yêu cầu đổi lịch giảng</Text>
        </View>
      </View>
          ...
          ListHeaderComponent={
            <View>
              <View style={styles.filterBox}>
                ...
                <Text style={styles.filterLabel}>Khoa quản lý</Text>
                <TouchableOpacity
                  style={styles.pickerField}
                  onPress={() => setPickerOpen('khoaQuanLy')}
                  disabled={loadingFilters}
                >
                  <Text
                    style={[
                      styles.pickerFieldText,
                      !selKhoa && styles.pickerFieldPlaceholder,
                    ]}
                    numberOfLines={1}
                  >
                    {selKhoa?.TEN || 'Tất cả khoa quản lý'}
                  </Text>
                  <MaterialIcons name="keyboard-arrow-down" size={20} color="#64748B" />
                </TouchableOpacity>
      ...
      />
      <FilterPickerSheet
        visible={pickerOpen === 'khoaQuanLy'}
        onClose={() => setPickerOpen(null)}
        data={khoaQuanLyList}
        selectedId={khoaQuanLyId}
        title="Chọn khoa quản lý"
        emptyLabel="Tất cả khoa quản lý"
        getLabel={(it) => it.TEN || it.ID}
        onSelect={(item) => {
          setKhoaQuanLyId(item.ID);
          setPickerOpen(null);
        }}
      />
      <FilterPickerSheet
        ...
        <View style={styles.sheetBackdrop}>
          <View style={styles.sheet}>
            <View style={styles.sheetHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.sheetTitle}>Xử lý {selectedIds.size} yêu cầu</Text>
                <Text style={styles.sheetSubtitle}>Chọn kết quả xử lý và (tùy chọn) ghi chú</Text>
              </View>
            ...
            <Text style={[styles.filterLabel, { marginTop: 12 }]}>Kết quả xử lý</Text>
            {submittingDuyet ? (
              <View style={[styles.centerBox, { paddingVertical: 24 }]}>
                <ActivityIndicator color="#1E3A8A" />
                <Text style={styles.muted}>Đang xử lý...</Text>
              </View>
            ) : (
              <View style={{ marginTop: 6 }}>
                {ketQuaXuLyList.map((tt, idx) => {
          ...
          <View style={styles.sheet}>
            ...
            {loadingChiTiet ? (
              ...
              <ScrollView style={{ maxHeight: 480 }}>
                ...
                <Row
                  label="Thời gian gửi"
                  value={chiTiet.NGAYTAO_DD_MM_YYYY_HHMMSS || chiTiet.NGAYTAO_DD_MM_YYYY}
                />
                ...
                <Row
                  label="Người duyệt"
                  value={chiTiet.NGUOIDUYET_TENDAYDU || chiTiet.NGUOIDUYET_TAIKHOAN}
                />
...
export default LecturerScheduleProcessingScreen;
