// Thời khóa biểu - Quản lý (admin) — port `lichgiangadmin.html` (reuse `lichgiang.js`).
// Khác bản cá nhân: thêm bộ chọn "Cán bộ" (search + picker) ở đầu trang, mọi API lịch
// dùng `strNhanSu_HoSoCanBo_Id` = ID cán bộ được chọn thay vì userId hiện tại.
// MVP: chỉ hiển thị lịch của cán bộ được chọn (không kèm điểm danh / đổi lịch — vì các
// thao tác đó chỉ có nghĩa với chính chủ lịch).
...
const LecturerScheduleAdminScreen = () => {
  ...
  const [searchingCanBo, setSearchingCanBo] = useState(false);
  const [selectedCanBo, setSelectedCanBo] = useState<CanBoItem | null>(null);
  ...
  // Load lịch khi đã có cán bộ + tuần thay đổi
  const loadLich = useCallback(async () => {
    if (!selectedCanBo) return;
    try {
      const data = await svc.getLichGiang(weekRange.startStr, weekRange.endStr, selectedCanBo.ID);
      setItems(data || []);
    } catch (e: any) {
  ...
  }, [selectedCanBo, weekRange.startStr, weekRange.endStr]);
  ...
  }, [loadLich, selectedCanBo]);

  // Search cán bộ. Nếu kết quả 1 phần tử → tự chọn (như web).
  const doSearch = async (autoPick: boolean) => {
    setSearchingCanBo(true);
    try {
      const list = await svc.searchCanBo(tuKhoa);
      setCanBoList(list);
      if (list.length === 0) {
        Alert.alert('Thông báo', 'Không tìm thấy cán bộ phù hợp.');
      } else if (autoPick && list.length === 1) {
        setSelectedCanBo(list[0]);
      } else {
        setPickerOpen(true);
      }
    } catch (e: any) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>TKB - Quản lý</Text>
          <Text style={styles.headerSubtitle}>Tra cứu lịch giảng theo cán bộ</Text>
        </View>
      </View>

      {/* Chọn cán bộ */}
      <View style={styles.canBoBox}>
        <Text style={styles.canBoLabel}>Cán bộ</Text>
        <View style={styles.searchRow}>
          <TextInput
            style={styles.searchInput}
            placeholder="Nhập mã / họ tên cán bộ..."
        ...
        {selectedCanBo ? (
          <TouchableOpacity
            ...
            onPress={() => canBoList.length > 1 && setPickerOpen(true)}
            activeOpacity={canBoList.length > 1 ? 0.7 : 1}
          >
            <View style={styles.selectedAvatar}>
              <Text style={styles.selectedAvatarText}>
                {(selectedCanBo.TEN || '?').charAt(0).toUpperCase()}
              </Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.selectedName} numberOfLines={1}>
                {selectedCanBo.HODEM} {selectedCanBo.TEN}
              </Text>
              <Text style={styles.selectedMeta} numberOfLines={1}>
                {selectedCanBo.MASO}
                {selectedCanBo.DAOTAO_COCAUTOCHUC_TEN
                  ? ` · ${selectedCanBo.DAOTAO_COCAUTOCHUC_TEN}`
                  : ''}
              </Text>
            </View>
          ...
          <Text style={styles.canBoHint}>
            Chưa chọn cán bộ. Nhập từ khóa rồi bấm tìm kiếm.
          </Text>
        )}
      </View>
      ...
      {!selectedCanBo ? (
        <View style={styles.centerBox}>
          ...
          <Text style={styles.muted}>Chọn cán bộ để xem thời khóa biểu</Text>
        </View>
      ) : loadingLich ? (
        <View style={styles.centerBox}>
          <ActivityIndicator size="large" color="#1E3A8A" />
          <Text style={styles.muted}>Đang tải lịch giảng...</Text>
        </View>
      ) : (
        <SectionList
          sections={sections}
      ...
      >
        ...
        >
          <View style={styles.pickerSheet}>
            <Text style={styles.pickerSheetTitle}>Chọn cán bộ ({canBoList.length})</Text>
            <FlatList
              data={canBoList}
              keyExtractor={(item, idx) => `${item.ID}_${idx}`}
              renderItem={({ item }) => {
                const active = selectedCanBo?.ID === item.ID;
                return (
                  <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  canBoLabel: { fontSize: 12, color: '#475569', fontWeight: '700' },
  canBoHint: { fontSize: 12, color: '#94A3B8', fontStyle: 'italic' },
...
export default LecturerScheduleAdminScreen;
