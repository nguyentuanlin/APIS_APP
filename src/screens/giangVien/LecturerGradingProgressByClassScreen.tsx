// Thống kê tiến độ nhập điểm theo đăng ký và theo lớp học phần — port `nhapdiemlophocphan.html` + `.js`.
...
import {
  lecturerGradingProgressByClassService as svc,
  KeHoachItem,
  KhoaQuanLyItem,
  HocPhanItem,
  DotThiItem,
  ...
  LopHocPhanItem,
  TienDoKetQua,
} from '../../services/giangVien/lecturerGradingProgressByClassService';
...
function FilterPickerSheet<T extends PickerLike>(props: {
  ...
  const labelOf = (it: T) =>
    getLabel ? getLabel(it) : it.TEN || it.TENKEHOACH || it.DAOTAO_KHOAQUANLY_TEN || it.ID;
...
const LecturerGradingProgressByClassScreen = () => {
  const navigation = useNavigation();

  const [keHoachList, setKeHoachList] = useState<KeHoachItem[]>([]);
  const [khoaList, setKhoaList] = useState<KhoaQuanLyItem[]>([]);
  const [hocPhanList, setHocPhanList] = useState<HocPhanItem[]>([]);
  const [dotThiList, setDotThiList] = useState<DotThiItem[]>([]);
  ...
  const [pickerOpen, setPickerOpen] = useState<null | 'keHoach' | 'khoa' | 'hocPhan' | 'dotThi'>(
    null
  );
  ...
  const [items, setItems] = useState<LopHocPhanItem[]>([]);

  const [activeRow, setActiveRow] = useState<LopHocPhanItem | null>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const [kh, dt] = await Promise.all([svc.getKeHoach(), svc.getDotThi()]);
        setKeHoachList(kh);
        setDotThiList(dt);
      } catch (e: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [khoa, hp] = await Promise.all([
          svc.getKhoaQuanLy(keHoachId),
          svc.getHocPhan(keHoachId, ''),
        ]);
        setKhoaList(khoa);
        setHocPhanList(hp);
        setKhoaId('');
        ...
        console.warn('[ProgressLHP] reload by keHoach:', e?.message);
      }
    })();
  }, [keHoachId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const hp = await svc.getHocPhan(keHoachId, khoaId);
        setHocPhanList(hp);
        setHocPhanId('');
      } catch (e: any) {
        console.warn('[ProgressLHP] reload by khoa:', e?.message);
      }
    })();
  }, [khoaId, keHoachId]);
  ...
  }, [tuKhoa, keHoachId, khoaId, hocPhanId, dotThiId]);

  const openChiTiet = async (row: LopHocPhanItem) => {
    setActiveRow(row);
    setChiTietValues({});
  ...
  const selKhoa = khoaList.find((it) => it.ID === khoaId);
  const selHocPhan = hocPhanList.find((it) => it.ID === hocPhanId);
  const selDotThi = dotThiList.find((it) => it.ID === dotThiId);

  const renderItem = ({ item, index }: { item: LopHocPhanItem; index: number }) => {
    ...
    return (
      ...
      >
        ...
        <Text style={styles.cardTitle} numberOfLines={2}>
          {item.TENLOP || item.MALOP || 'Lớp học phần'}
        </Text>
        {!!item.MALOP && (
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSubtitle}>Theo đăng ký và theo lớp học phần</Text>
        </View>
      </View>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View style={styles.filterBox}>
              ...
              <TouchableOpacity
                style={styles.pickerField}
                onPress={() => setPickerOpen('khoa')}
              >
                <Text
                  style={[styles.pickerFieldText, !selKhoa && styles.pickerFieldPlaceholder]}
                  numberOfLines={1}
                >
                  {selKhoa?.DAOTAO_KHOAQUANLY_TEN || 'Tất cả khoa quản lý'}
                </Text>
                <MaterialIcons name="keyboard-arrow-down" size={20} color="#64748B" />
              </TouchableOpacity>
              ...
              >
                <Text
                  style={[styles.pickerFieldText, !selDotThi && styles.pickerFieldPlaceholder]}
                  numberOfLines={1}
                >
              ...
              {items.length > 0 && (
                <Text style={styles.summaryText}>{items.length} lớp học phần</Text>
              )}
            </View>
          }
          ListEmptyComponent={
      ...
      />
      <FilterPickerSheet
        visible={pickerOpen === 'khoa'}
        onClose={() => setPickerOpen(null)}
        data={khoaList}
        selectedId={khoaId}
        title="Chọn khoa quản lý"
        emptyLabel="Tất cả khoa quản lý"
        getLabel={(it) => it.DAOTAO_KHOAQUANLY_TEN || it.ID}
        onSelect={(item) => {
          setKhoaId(item.ID);
          setPickerOpen(null);
        }}
            ...
            <View style={styles.sheetHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.sheetTitle} numberOfLines={2}>
                  {activeRow?.TENLOP || activeRow?.MALOP || 'Chi tiết tiến độ'}
                </Text>
...
export default LecturerGradingProgressByClassScreen;
